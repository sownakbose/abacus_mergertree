...
outputs_now = glob.glob(odir + "treenode_associations*.asdf")
...
if len(outputs_now) > 0:
	...
	snapList   = snapList[arg:]
	arg = np.argmin(abs(z_interest-z_now))

	z_interest = z_interest[arg:]
	z_match    = z_match[arg:]
	sys.stdout.flush()
...
def surf_halo(iter, neigh, mainProgArray, mainProgFracArray):
	...
	for	j in range(len(neigh)):
		...
		if (ntag_next[idx_this_cand] == 0) or (ntag_next[idx_this_cand] <= lowlim) or (mhalo_next[idx_this_cand]/mhalo[halo_index] < 0.05):
			continue
...
def surf_halo_dnext(iter, neigh, dmainProgArray, dmainProgFracArray):
	...
	for	j in range(len(neigh)):
		...
		if (idx_this_cand == len(mhalo_dnext)):
			break # No object after this will be eligible
		if (ntag_dnext[idx_this_cand] == 0) or (ntag_dnext[idx_this_cand] <= lowlim) or (mhalo_dnext[idx_this_cand]/mhalo[halo_index] < 0.02):
			continue
		...
		ptr[ptr<0]      = 0
		mask            = np.where(ids_tmp_cand[ptr] == ids_this_halo)[0]
		#print(len(mask), indxx_dnext[idx_this_cand])
		# The contribution of this candidate is the number of matched particles weighted by density
		weighted_count = np.sum(rho_this_halo[mask])
...
def surf_halo_tot(iter, counter, mainProgArray, mainProgFracArray, dmainProgArray, dmainProgFracArray):
		...
		try:
			surf_halo(iter, neigh, mainProgArray, mainProgFracArray)
		except ValueError:
			surf_halo(iter, [-999], mainProgArray, mainProgFracArray)#[0]
...
def surf_halo_final(iter, counter, mainProgArray, mainProgFracArray):
	...
	try:
		surf_halo(iter, neigh, mainProgArray, mainProgFracArray, isSplitArray)
	except ValueError:
		surf_halo(iter, [-999], mainProgArray, mainProgFracArray, isSplitArray)#[0]
...
for jj in range(num_epochs):
	...
	step_arg      = np.argmin( abs(snapList-z_interest[jj])  )
	step          = steps[step_arg]
	step_arg_next = np.argmin( abs(snapList-z_match[jj][0])  )
	step_next     = steps[step_arg_next]
	...
	if do_dnext:
		...
		step_arg_dnext = np.argmin( abs(snapList-z_match[jj][1])  )
		step_dnext     = steps[step_arg_dnext]
	...
	for ifile_counter in range(ichunk_start, ichunk_finish):
		...
		header.update( {"z_next": z_match[jj][0], "z_dnext": z_match[jj][1]} )

		if do_dnext:
			with Parallel(n_jobs = num_cores, batch_size = batch_size, pre_dispatch = pre_dispatch, backend = "multiprocessing") as parallel:
				parallel(delayed(surf_halo_tot)(i, counter, MAIN_PROG, MPMATCH_FRAC, DMAIN_PROG, DMPMATCH_FRAC) for i, counter in zip(range(len(mask_eligible)), trange(len(mask_eligible), disable=None))     )

		else:
			with Parallel(n_jobs = num_cores, batch_size = batch_size, pre_dispatch = pre_dispatch, backend = "multiprocessing") as parallel:
				parallel(delayed(surf_halo_final)(i, counter, MAIN_PROG, MPMATCH_FRAC) for i, counter in zip(range(len(mask_eligible)), trange(len(mask_eligible), disable=None))     )
