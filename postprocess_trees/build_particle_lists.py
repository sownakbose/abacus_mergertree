...
def fill_particles(
    ...
    if isPrimary:
        for nn in range(len(extra_halo_global_index)):
            ...
            rvint_A[counter_A : counter_A + noutA[halo_now]] = RV[
                nstartA[halo_now] : nstartA[halo_now] + noutA[halo_now]
            ]
            # Add subsample A pids
            pid_A[counter_A : counter_A + noutA[halo_now]] = PID[
                nstartA[halo_now] : nstartA[halo_now] + noutA[halo_now]
            ]
...
for i, ii in enumerate(range(start_file_num, nfiles_to_do)):
    ...
    try:
        ...
        tot_particles_to_add = int(
            np.max((np.sum(noutA[indices_that_merge_this_slab]), np.sum(noutB[indices_that_merge_this_slab]))) * 1.2
        )  # 20% buffer
    ...
    for jj in range(1, len(unq_haloidx)):
        ...
        counter_A, counter_B = fill_particles(
            extra_halo_global_index,
            counter_A,
            counter_B,
            rvint_A,
    ...
    for jj in range(1, len(unq_haloidx)):
        ...
        N_mainprog[indices_this_slab[haloid]] = combine_mainprog(
            extra_halo_global_index, N_mainprog_thishalo_initial, N_mainprog
        )
