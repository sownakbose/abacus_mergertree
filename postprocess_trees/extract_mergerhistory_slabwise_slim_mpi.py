...
COMPRESSION_OPTIONS = dict(
    typesize='auto', shuffle='shuffle', compression_block_size=12 * 1024**2, blosc_block_size=3 * 1024**2, nthreads=4
)
...
for i, ii in enumerate(range(nfiles)):
    ...
    for jj in range(1, Nsnapshot):
        ...
        cat_list_next = return_search_list(
            simdir + '/halos/z%4.3f/halo_info/' % (halo_snapList[jj]) + 'halo_info*.asdf', ii
        )
        t0 = time.time()
        ...
        write_queue.write(
            odir + '/temporary_index_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
            mbp_idx.data[numhalos[0] : numhalos[0] + numhalos[1]],
        )
        write_queue.write(
            odir + '/temporary_mass_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
            mbp_mass.data[numhalos[0] : numhalos[0] + numhalos[1]],
        )
        write_queue.write(
            odir + '/temporary_vmax_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
            mbp_vmax.data[numhalos[0] : numhalos[0] + numhalos[1]],
        )
        write_queue.write(
            odir + '/temporary_vrms_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
            mbp_vrms.data[numhalos[0] : numhalos[0] + numhalos[1]],
        )
        write_queue.write(
            odir + '/temporary_prog_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
            MainProg.data[numhalos[0] : numhalos[0] + numhalos[1]],
        )
        if jj == 1:
            write_queue.write(
                odir + '/temporary_vel_matches_z%4.3f.%03d.npy' % (snapList[jj], ii),
                mbp_vel.data[numhalos[0] : numhalos[0] + numhalos[1]],
            )
        ...
        if nslab > 1:
            if ii == 0:
                file_prev = nfiles - 1
                write_queue.write(
                    odir + '/temporary_index_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                    mbp_idx.data[: numhalos[0]],
                )
                write_queue.write(
                    odir + '/temporary_mass_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                    mbp_mass.data[: numhalos[0]],
                )
                write_queue.write(
                    odir + '/temporary_vmax_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                    mbp_vmax.data[: numhalos[0]],
                )
                write_queue.write(
                    odir + '/temporary_vrms_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                    mbp_vrms.data[: numhalos[0]],
                )
                write_queue.write(
                    odir + '/temporary_prog_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                    MainProg.data[: numhalos[0]],
                )
                if jj == 1:
                    write_queue.write(
                        odir + '/temporary_vel_matches_z%4.3f.%03d.npy' % (snapList[jj], file_prev),
                        mbp_vel.data[: numhalos[0]],
                    )
                file_next = ii + 1
                write_queue.write(
                    odir + '/temporary_index_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                    mbp_idx.data[numhalos[0] + numhalos[1] :],
                )
                write_queue.write(
                    odir + '/temporary_mass_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                    mbp_mass.data[numhalos[0] + numhalos[1] :],
                )
                write_queue.write(
                    odir + '/temporary_vmax_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                    mbp_vmax.data[numhalos[0] + numhalos[1] :],
                )
                write_queue.write(
                    odir + '/temporary_vrms_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                    mbp_vrms.data[numhalos[0] + numhalos[1] :],
                )
                write_queue.write(
                    odir + '/temporary_prog_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                    MainProg.data[numhalos[0] + numhalos[1] :],
                )
                if jj == 1:
                    write_queue.write(
                        odir + '/temporary_vel_matches_z%4.3f.%03d.npy' % (snapList[jj], file_next),
                        mbp_vel.data[numhalos[0] + numhalos[1] :],
                    )
