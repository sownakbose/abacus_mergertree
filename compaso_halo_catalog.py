...
class CompaSOHaloCatalog:
    ...
    def __init__(self, path, load_subsamples=False, convert_units=True, unpack_bits=False, fields='all'):
        ...
        unpack_bits: bool or str, optional
            ...
            be extracted.
            ``merger`` would load only particle IDs and current local density
            Default: False.
        ...
        fields: str or list of str, optional
            ...
            ``merger`` would load only the subset of the halo catalog needed for
    ...
    def _read_halo_info(self, halo_fns, load_fields):
        ...
        if load_fields == 'all':
            ...
            cols = {col:np.empty(N_halos, dtype=user_dt_merger[col]) for col in user_dt_merger.names}
    ...
    def _load_pids(self, unpack_bits, N_halo_per_file, check_pids=False):
        ...
        if unpack_bits == True:
            ...
            justpid, lagr_pos, tagged, density = unpack_pids(pids_AB, unpackbox, self.header['ppd'], unpack_type = 'All')
        ...
        elif unpack_bits == 'merger':
            # unpack_pids will do unit conversion if requested
            unpackbox = self.header['BoxSize'] if self.convert_units else 1.
            justpid, lagr_pos, tagged, density = unpack_pids(pids_AB, unpackbox, self.header['ppd'], unpack_type = 'merger')
            self.subsamples.add_column(density, name='density', copy=False)
            self.subsamples.add_column(justpid, name='pid', copy=False)
        else:
