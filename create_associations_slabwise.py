...
from merger_tree_library import indxxHaloSlabwise, read_halo_catalogue
...
my_parser = argparse.ArgumentParser(
    description='Code to create halo time slice associations from abacus outputs',
...
my_parser.add_argument(
    '-simname', action='store', type=str, help='Simulation name', default='AbacusSummit_base_c000_ph006'
)
my_parser.add_argument(
    '-num_epochs', action='store', type=int, help='Number of epochs to generate associations for', default=1
)
my_parser.add_argument(
    '-num_slabs_todo',
    action='store',
...
my_parser.add_argument('-num_cores', action='store', type=int, help='Number of cores for parallelisation', default=16)
my_parser.add_argument(
    '-outputdir',
    action='store',
    type=str,
...
if (halo_type == 'Abacus_SO') or (halo_type == 'Abacus_FOF'):
    ...
    (halo_type == 'Rockstar')
    or (halo_type == 'Abacus_Cosmos')
    or (halo_type == 'Abacus_asdf')
    or (halo_type == 'Abacus_slabwise')
):
...
if len(outputs_now) > 0:
    ...
    steps = steps[arg:]
    print(
        'Found existing outputs in %s! Will resume calculation from z=%4.3f, superslab number %d.'
...
if num_epochs > len(steps) - 1:
    ...
    print(
        'num_epochs (%d) is greater than the number of remaining steps (%d). Resetting to %d.'
        % (args.num_epochs, len(steps), num_epochs)
    )
    sys.stdout.flush()
...
def surf_halo_dnext(iter, neigh, dmainProgArray, dmainProgFracArray):
    ...
    for j in range(len(neigh)):
        ...
        ids_tmp_candA = pids_dnext[
            nstartA_dnext[idx_this_cand] : nstartA_dnext[idx_this_cand] + ntagA_dnext[idx_this_cand]
        ]
        ids_tmp_candB = pids_dnext[
            nstartB_dnext[idx_this_cand] : nstartB_dnext[idx_this_cand] + ntagB_dnext[idx_this_cand]
        ]
        ids_tmp_cand = np.append(ids_tmp_candA, ids_tmp_candB)
        ids_tmp_cand.sort()
...
for jj in range(num_epochs):
    ...
    for ifile_counter in range(ichunk_start, ichunk_finish):
        ...
        (
            ...
            pids_next,
            rho_next,
        ) = read_halo_catalogue(search_list_next, halo_type, return_header=False)
        t_read_1 = time.time()
        ...
        if do_dnext:
            ...
            (
                ...
                pids_dnext,
                rho_dnext,
            ) = read_halo_catalogue(search_list_dnext, halo_type, return_header=False)
            t_read_1 = time.time()
        ...
        neighbours = tree.query(
            pos[mask_eligible] + half_box, distance_upper_bound=search_rad, k=num_neigh, workers=-1
        )[1]
        t_query_2 = time.time()
        ...
        if do_dnext:
            ...
            sys.stdout.flush()
            t_query_1 = time.time()
            dneighbours = tree_dnext.query(
                pos[mask_eligible] + half_box, distance_upper_bound=search_rad, k=75, workers=-1
            )[1]
            t_query_2 = time.time()
            tquery = t_query_2 - t_query_1
            print('Took %4.2fs to query all neighbours.' % (tquery))
        ...
        if do_dnext:
            with Parallel(
                n_jobs=num_cores, batch_size=batch_size, pre_dispatch=pre_dispatch, backend='multiprocessing'
            ) as parallel:
                PROG_INDX = parallel(
                    delayed(surf_halo_tot)(i, counter, MAIN_PROG, MPMATCH_FRAC, IS_SPLIT, DMAIN_PROG, DMPMATCH_FRAC)
                    for i, counter in zip(range(len(mask_eligible)), trange(len(mask_eligible), disable=None))
                )
            ...
            with Parallel(
                ...
                PROG_INDX = parallel(
                    delayed(surf_halo_final)(i, counter, MAIN_PROG, MPMATCH_FRAC, IS_SPLIT)
                    for i, counter in zip(range(len(mask_eligible)), trange(len(mask_eligible), disable=None))
                )
